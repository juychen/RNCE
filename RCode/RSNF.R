...
.rdominateset <- function(xx,KK=20) {
  ###This function outputs the top KK neighbors.	
  
  krnn_mask <- k_reciprocal_nn(xx,KK)
...
.maskdominateset <- function(xx,mask,KK=20) {
  ###This function outputs the top KK neighbors.	
...
MSNF <- function(Wall, K=20, t=20) {
  ...
  MHi <- matrix(1, nrow(Wall[[1]]), ncol(Wall[[1]]))
  MLo <- matrix(0, nrow(Wall[[1]]), ncol(Wall[[1]]))
  
  
  
  for(i in 1:LW){
    
    Mnei <- k_reciprocal_nn(Wall[[i]],KK=K)
  ...
  ### Calculate the local transition matrix. (KNN step?)
  for(i in 1:LW){
    newW[[i]] <- (.maskdominateset(Wall[[i]],MLo, K))
  }
