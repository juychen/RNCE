...
compConcordIndx <- function(allPairs)
{
     ...
     if(length(allPairs)>9){
       ...
       LDNFS <- (length(allPairs)-9)
       for(i in 1:LDNFS){
         pairname <- paste("dnf",i,"Pairs",sep='')
         extraCindex[[pairname]] = Hmisc::rcorr.cens(x= allPairs[[pairname]][[3]], S=allPairs$benchPairs$bench) 
       }
     }
     ...
     if(length(allPairs)>9){
       ...
       for(i in 1:LDNFS){
         pairname <- paste("dnf",i,"Pairs",sep='')
         extraPval[[pairname]] <- cindexComp2(integrCindex, extraCindex[[pairname]], allPairs$integrPairs$obs.combiall,allPairs[[pairname]][[3]])
         
         cname <- paste("dnf",i,"Cindex",sep='')
         pname <- paste("dnf",i,"Pval",sep='')
         cindxLst[[cname]] <- extraCindex[[i]]["C Index"]
         pVals[[pname]] <- extraPval[[i]]$p.value
         }
     }
